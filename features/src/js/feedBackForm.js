(() => {
  ...
  document.getElementById("saveButton").addEventListener("click", () => {
    ...
    const improvement = Array.from(
      document.getElementById("improvement").selectedOptions,
  ...
  window.addEventListener("load", () => {
    ...
    if (improvement && improvement !== "null") {
      ...
      improvementArray.forEach((value) => {
        const option = Array.from(improvementSelect.options).find(
          (opt) => opt.value === value,
        );
        if (option) {
  ...
  document.getElementById("clearButton").addEventListener("click", () => {
    const userConfirmed = confirm(
      "Are you sure you want to clear the form progress? This action cannot be undone.",
    );
    if (userConfirmed) {
...
function toggleOtherTextarea() {
  ...
  const selectedOptions = Array.from(improvementSelect.selectedOptions).map(
    (option) => option.value,
  );
  otherTextContainer.style.display = selectedOptions.includes("others")
    ? "block"
    : "none";
}
  ...
  if (event.key === "Escape") {
    ...
    if (feedbackForm.contains(activeElement) && activeElement.id) {
      ...
      if (
        ...
        Array.from(activeElement.options).forEach(
